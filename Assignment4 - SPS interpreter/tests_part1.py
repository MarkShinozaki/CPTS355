...
class HW4Sampletests_part1(unittest.TestCase):
    ...
    def test_define1(self):
        # if the dictstack is empty, define should add a new  dictionary to dictstack before defining the new variable
        self.psstacks.define("/n1", 4)
        self.assertEqual(self.psstacks.lookup("n1"),4)

    def test_define2(self):
        # if the dictstack is empty, define should add a new dictionary before defining the new variable
        self.psstacks.define("/n1", 4)
    ...
    def test_array(self):
        ...
        self.psstacks.opPush(5)
        self.psstacks.array()
        arr = self.psstacks.opPop()
        self.assertTrue(isinstance(arr, ArrayValue))
        self.assertEqual(len(arr.value),5)   #make sure the array length is same as the given size
        self.assertEqual(arr.value, [None,None,None,None,None])
