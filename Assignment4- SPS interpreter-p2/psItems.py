...
class Array(Expr):
    ...
    def evaluate(self,psstacks):
        myArrayValue = ArrayValue(self.value)
        myArrayValue.apply(psstacks)
...
class Name(Expr):
    ...
    def evaluate(self, pssstacks):
        """`ps_env` is the PSOperators object that include the `opstack` and `dictstack`. """
        if self.var_name[0] =='/':
            pssstacks.opPush(self.var_name)
        elif self.var_name in pssstacks.builtin_operators:
            pssstacks.builtin_operators[self.var_name]()
        else:
            tmp = pssstacks.lookup(self.var_name)
            if tmp == None:
                print("Tried to look up variable name that's not in dict stack")
            else:
                if isinstance(tmp,FunctionValue):
                    tmp.apply(pssstacks)
                else:
                    pssstacks.opPush(tmp)   
    ...
    def apply(self, psstacks):
        ...
        while True:
            ...
            if x != '[':
                ...
                self.value = list(reversed(newlist))
                psstacks.opPush(self)
                break
...
class FunctionValue(Value):
    ...
    def apply_function(self, psstacks,index):
        expr_list = self.body
        psstacks.dictPush(index,{})##put tjis in fanction value
        for expr in expr_list:
            expr.evaluate(psstacks) 
        psstacks.dictPop() 
