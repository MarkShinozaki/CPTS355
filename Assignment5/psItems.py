...
class Literal(Expr):
    ...
    def evaluate(self, psstacks):
        # push evaluated value to opstack
...
class Array(Expr):
    ...
    #needs to be updated
    def evaluate(self,psstacks):
        init_count = len(psstacks.opstack)
        # call the evaluate method using value in the value array
        for val in self.value:
            val.evaluate(psstacks)
        
        # pop the evaluated values and store them in result list
        result = []
        for val in range(len(psstacks.opstack) - init_count):
            result.append(psstacks.opPop())

        # create a ArrayValue object and push it to the opstack
        psstacks.opPush(ArrayValue(list(reversed(result))))
...
class Name(Expr):
    ...
    def evaluate(self,psstacks):
        # If the `Name` represents a name constant (i.e., its `var_name` attribute starts with a `/`), it will be evaluated to a Python string having value `var_name` . The evaluated value will be pushed onto the opstack.
        if self.var_name[0] == '/':
            psstacks.opPush(self.var_name)
        
        # If the `Name` represents a built-in operator (i.e., its `var_name` attribute is one of the built-in operator names),  then we will evaluate it by executing the operator function defined in stacks.py in the current environment (opstack).
        elif self.var_name in psstacks.builtin_operators:
            #print(self.var_name)
            psstacks.builtin_operators[self.var_name]()

        # If the `Name` represents a variable or function, interpreter looks up the value of the variable in the current environment (dictstack).
        else:
            # get variable value
            dictVal = psstacks.lookup(self.var_name)
            # If the variable value is a function (`FunctionValue`), it should be applied (i.e., executed) by calling its `apply` method. 
            if isinstance(dictVal, FunctionValue):
                psstacks.dictPush((self.staticLink(psstacks, self.var_name), {}))
                dictVal.apply(psstacks)
            # Otherwise, the variable value is a constant and it should be pushed onto the opstack. 
            else:
                #print(dictVal)
                psstacks.opPush(dictVal)

    def staticLink(self, psstacks, fname):
        fname = '/' + fname
        for i in range (len(psstacks.dictstack)):
            # search the index where function is defined in dictstack
            if fname in psstacks.dictstack[i][1]:
                return i
...
class FunctionValue(Value):
    ...
    def apply(self, psstacks):
        ...
        # by calling token's evaluate method
        for token in self.body:
            token.evaluate(psstacks)
        # AR will be popped from the dictstack when execution is complete
        psstacks.dictPop()
